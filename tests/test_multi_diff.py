...
class MockUI:
    ...
    def show_info(self, msg): self.infos.append(msg)
    def show_diff(self, filename, orig, mod): self.diffs.append((filename, orig, mod))
    def show_error(self, msg): self.errors.append(msg)
    def render_message(self, role, content, model_name=None): self.messages.append((role, content))
    def show_tool_result(self, res): self.tool_results.append(res)
    def show_status(self, msg):
        from contextlib import contextmanager
        @contextmanager
        def status_mgr(): yield
        return status_mgr()
...
class MockProvider(LLMProvider):
    ...
    def upload_file(self, file_path, mime_type): return None
...
def test_multiple_diffs_handling(tmp_path):
    ...
    p1 = MessagePart(type="tool_call", tool_name="apply_diff", tool_args={
        "filename": str(file1),
        "diff": f"--- {file1}\n+++ {file1}\n@@ -1,2 +1,2 @@\n-line1\n+line1_new\n line2"
    })
    p2 = MessagePart(type="tool_call", tool_name="apply_diff", tool_args={
        "filename": str(file2),
        "diff": f"--- {file2}\n+++ {file2}\n@@ -1,2 +1,2 @@\n-alpha\n+alpha_new\n beta"
    })
    
    # Response with 2 tool calls, then final text
    resp1 = ProviderResponse(text="", parts=[p1, p2], input_tokens=10, output_tokens=10, total_tokens=20)
    resp2 = ProviderResponse(text="Done!", parts=[MessagePart(type="text", text="Done!")], input_tokens=5, output_tokens=5, total_tokens=10)
