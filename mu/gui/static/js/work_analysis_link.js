(() => {
    ...
    function renderCostSemantics(modelApi) {
        ...
        if (status === 'unpriced') {
            ...
            value.title = `Provider/model spend is plan-based or missing a stable tariff${billing ? ` (${billing})` : ''}. This is not $0.`;
        } else if (status === 'partial') {
            value.textContent = `$${amount.toFixed(2)} + ?`;
            value.title = `${unpriced} attempt${unpriced === 1 ? '' : 's'} have unpriced/plan-based model spend.`;
            ...
            value.textContent = `$${amount.toFixed(2)}`;
            value.title = 'Attributed model/API spend. Workspace compute/storage/network cost is separate.';
        }
    }
