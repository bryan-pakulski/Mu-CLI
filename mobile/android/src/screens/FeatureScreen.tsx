...
function PhaseCard({ phase, expanded, onToggle, onAdvance, colors }: {
  phase: FeaturePhase; expanded: boolean; onToggle: () => void; onAdvance: (taskId: number) => void; colors: any;
}) {
    ...
    <Card style={{ marginBottom: spacing.sm }}>
      ...
      {expanded && (
        <View style={{ marginTop: spacing.sm, paddingTop: spacing.sm, borderTopWidth: 0.5, borderTopColor: colors.border }}>
          {phase.tasks.map(task => <TaskRow key={task.id} task={task} onAdvance={onAdvance} colors={colors} />)}
        </View>
      )}
    </Card>
  );
...
function TaskRow({ task, onAdvance, colors }: {
  task: FeatureTask; onAdvance: (id: number) => void; colors: any;
}) {
  return (
