...
| `auto_compaction_enabled` | bool | `true` | Automatically roll older history into L2 throughout a task, including multiple times within one turn. Saved `false` values remain off; provider-overflow recovery always remains enabled. |
| `auto_compaction_token_limit` | int | `64000` | Soft L5 working-history threshold, capped further by the provider-aware history budget. Above it, automatic cleanup aims for half that budget. `0` uses only the provider-aware threshold. Recent/protected results can keep history above the target. |
