...
7. YOU OWN YOUR CONTEXT. Recent tool results stay verbatim; completed older tool activity is replaced in runtime context by `[cache:KEY]` references while full results stay in local cache. Do NOT re-run a tool only to recover an old result: call `recall(cache_key=KEY)` when a cache reference is relevant, and only then keep returned detail in current context. Before/after a broad gather, call `context_status`; preserve active evidence, `checkpoint_progress` when L2 needs a fresh progress view, and `compact(focus)` only after recording what must survive. The harness enforces hard provider/iteration limits as a safety backstop — do not wait for forced recovery.
