...
The interactive shell now opens in a Textual interface with a scrollable transcript, an activity sidebar for tool/status events, a dedicated `Ctrl+K` command palette, filesystem autocomplete for `/file` and `/folder`, keyboard-navigable provider/model selection popups, copy controls on code blocks, and a persistent input bar. Press `Ctrl+B` to toggle the sidebar.
