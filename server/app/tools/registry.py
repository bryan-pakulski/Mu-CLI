...
from server.app.workspace.base import ToolResult
...
class ReadFileTool(ToolDefinition):
    ...
    def _resolve(self, path_value: str) -> tuple[Path | None, str | None]:
        ...
        target = (
            (root / raw).resolve() if root is not None and not raw.is_absolute() else raw.resolve()
        )
        if root is not None:
...
class GitTool(ToolDefinition):
    name = "git"
    description = (
        "Run a constrained git operation (status, diff, log, add, commit, branching, pull request)."
    )
    mutating = True
    ...
    def run(self, args: dict) -> ToolResult:
        ...
        if op in self.SAFE_OPS:
            ...
            if diff_proc.returncode != 0 or log_proc.returncode != 0:
                ...
                return ToolResult(
                    ok=False, output=error.strip() or "Unable to inspect branch changes"
                )
            output = "\n".join([
        ...
        proc = subprocess.run(
            command, text=True, capture_output=True, cwd=str(root) if root is not None else None
        )
...
class ListWorkspaceFilesTool(ToolDefinition):
...
class GetWorkspaceFileContextTool(ToolDefinition):
...
class ListUploadedContextFilesTool(ToolDefinition):
    ...
    def run(self, args: dict) -> ToolResult:
        ...
        files = [
            item
            for item in sorted(session_dir.iterdir(), key=lambda x: x.name.lower())
            if item.is_file()
        ]
        if not files:
            return ToolResult(ok=True, output="No uploaded context files.")
...
class GetUploadedContextFileTool(ToolDefinition):
...
class ClearUploadedContextStoreTool(ToolDefinition):
...
class FetchUrlContextTool(ToolDefinition):
    ...
    def run(self, args: dict) -> ToolResult:
        ...
        text = (
            self._html_to_text(body) if "html" in content_type or "<html" in body.lower() else body
        )
        return ToolResult(ok=True, output=text[:max_chars])


class SearchWebContextTool(ToolDefinition):
    ...
    def _request_json(url: str) -> tuple[dict | None, str | None]:
        req = urllib.request.Request(
            url, headers={"User-Agent": "mu_cli/1.0 (+grounding-tool)"}, method="GET"
        )
        try:
    ...
    def _search_google(self, query: str, max_results: int) -> ToolResult:
        ...
        if not api_key or not cse_id:
            return ToolResult(
                ok=False,
                output="Google grounding unavailable: set GOOGLE_CSE_API_KEY and GOOGLE_CSE_ID",
            )
    ...
    def _search_duckduckgo(self, query: str, max_results: int) -> ToolResult:
                    ...
                    if (
                        ...
                        for nested in topic["Topics"]:
                            if isinstance(nested, dict):
                                rows.append((
                                    str(nested.get("Text", ""))[:80],
                                    str(nested.get("FirstURL", "")),
                                    str(nested.get("Text", "")),
                                ))
                    elif isinstance(topic, dict):
                        rows.append((
                            str(topic.get("Text", ""))[:80],
                            str(topic.get("FirstURL", "")),
                            str(topic.get("Text", "")),
                        ))
        ...
        if not rows:
            ...
            req = urllib.request.Request(
                html_url, headers={"User-Agent": "mu_cli/1.0 (+grounding-tool)"}, method="GET"
            )
            try:
                with urllib.request.urlopen(req, timeout=12) as resp:
    ...
    def run(self, args: dict) -> ToolResult:
        ...
        if duck.ok:
            return duck
        return ToolResult(
            ok=False, output=f"auto search failed; google={google.output}; duckduckgo={duck.output}"
        )


class ExtractLinksContextTool(ToolDefinition):
...
class SearchArxivPapersTool(ToolDefinition):
    ...
    def run(self, args: dict) -> ToolResult:
        ...
        req = urllib.request.Request(
            url, headers={"User-Agent": "mu_cli/1.0 (+research-tool)"}, method="GET"
        )
        try:
            with urllib.request.urlopen(req, timeout=12) as resp:
...
class FetchPdfContextTool(ToolDefinition):
    ...
    def run(self, args: dict) -> ToolResult:
        ...
        if PdfReader is None:
            return ToolResult(
                ok=False, output=f"PDF parsing unavailable: {module_name}.PdfReader not found"
            )
...
class ScoreSourcesTool(ToolDefinition):
    name = "score_sources"
    description = (
        "Score research sources for reputation, recency, primary/secondary signal, and duplication."
    )
    mutating = False
    requires_approval = False
    schema = {
    ...
    def _primary_secondary_score(title: str, snippet: str, url: str) -> tuple[str, float]:
        ...
        if any(
            token in text for token in ["survey", "review", "overview", "blog", "news", "opinion"]
        ):
            return "secondary", 0.55
        if any(
            token in domain
            for token in ["nature.com", "science.org", "arxiv.org", "acm.org", "ieee.org"]
        ):
            return "primary", 0.9
    ...
    def run(self, args: dict) -> ToolResult:
        ...
        for source in normalized:
            ...
            source_type, source_type_score = self._primary_secondary_score(
                source["title"], source["snippet"], source["url"]
            )
            duplicate_count = domains.get(domain, 0)
            duplication_penalty = (
                0.0 if duplicate_count <= 1 else min(0.25, 0.08 * (duplicate_count - 1))
            )
            score = max(
                0.0,
                min(
...
class CustomCommandTool(ToolDefinition):
...
class MakefileAgentTool(ToolDefinition):
    ...
    def run(self, args: dict) -> ToolResult:
        ...
        if targets and target not in targets:
            ...
            return ToolResult(
                ok=False, output=f"Unknown Makefile.agent job '{target}'. Known jobs: {formatted}"
            )

        cmd = ["make", "-f", str(makefile), target, *raw_args]
        proc = subprocess.run(
            cmd, text=True, capture_output=True, cwd=str(root), timeout=120, check=False
        )
        output = (proc.stdout or "") + ("\n" + proc.stderr if proc.stderr else "")
        if proc.returncode != 0:
